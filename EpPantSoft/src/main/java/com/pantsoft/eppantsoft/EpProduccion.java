...
public class EpProduccion extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws javax.servlet.ServletException, IOException {
		...
		try {
			...
			if (ep.esMetodo("orden_actualizarHabilitacion") && ep.esVersion("v1")) {
				SerOrden serOrden = ep.getObjetFromBody(SerOrden.class);
				new PmOrden().actualizarHabilitacion(serOrden);
				ep.voidEnBody();
				return;
			}
			...
			if (ep.esMetodo("orden_actualizarModelo") && ep.esVersion("v1")) {
				SerOrden serOrden = ep.getObjetFromBody(SerOrden.class);
				serOrden = new PmOrden().actualizarModelo(serOrden);
				ep.objectEnBody(serOrden);
				return;
			}
			if (ep.esMetodo("orden_terminarOrden") && ep.esVersion("v1")) {
				SerOrden serOrden = ep.getObjetFromBody(SerOrden.class);
				serOrden = new PmOrden().terminarOrden(serOrden);
				ep.objectEnBody(serOrden);
				return;
			}
			if (ep.esMetodo("orden_actualizarFaltanteTela") && ep.esVersion("v1")) {
				SerOrden serOrden = ep.getObjetFromBody(SerOrden.class);
				serOrden = new PmOrden().actualizarFaltanteTela(serOrden);
				ep.objectEnBody(serOrden);
				return;
			}
			...
			if (ep.esMetodo("ordenProceso_actObsRevision") && ep.esVersion("v1")) {
				SerOrdenProceso serOrdenProceso = ep.getObjetFromBody(SerOrdenProceso.class);
				new PmOrden().actualizarOrdenProceso_ObsRevision(serOrdenProceso);
				ep.voidEnBody();
				return;
			}
