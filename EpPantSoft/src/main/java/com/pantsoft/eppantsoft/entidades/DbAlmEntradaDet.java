...
public class DbAlmEntradaDet extends ClsEntidad {
	...
	public DbAlmEntradaDet(SerAlmEntradaDet serAlmEntradaDet) throws ExcepcionControlada {
		Key key = KeyFactory.createKey("DbAlmEntradaDet", serAlmEntradaDet.getEmpresa() + "-" + serAlmEntradaDet.getFolioAlmEntrada());
		...
		setAlmacen(serAlmEntradaDet.getAlmacen());
		setTemporada(serAlmEntradaDet.getTemporada());
		setModelo(serAlmEntradaDet.getModelo());
		setDia(serAlmEntradaDet.getDia());
		setMes(serAlmEntradaDet.getMes());
		setCantidad(serAlmEntradaDet.getCantidad());
		setDetalle(serAlmEntradaDet.getDetalle());
	}
	...
	public List<ClsCampo> getCampos() {
		return Arrays.asList(empresa, folioAlmEntrada, almacen, temporada, modelo, dia, mes, cantidad, detalle);
	}

	public SerAlmEntradaDet toSerAlmEntradaDet() throws ExcepcionControlada {
		return new SerAlmEntradaDet(getEmpresa(), getFolioAlmEntrada(), getAlmacen(), getTemporada(), getModelo(), getDia(), getMes(), getCantidad(), getDetalle());
	}
